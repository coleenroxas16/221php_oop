...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($_POST['search'])) {
        ...
        try {
            ...
            if ($connection) {
                // SQL query with JOIN
                $query = $connection->prepare("SELECT users.user_id, users.firstname, users.lastname, users.birthday, users.sex, users.user_name, users.user_profile_picture, CONCAT(user_address.user_city,', ', user_address.user_province) AS address FROM users INNER JOIN user_address ON users.user_id = user_address.user_id WHERE users.user_name LIKE ? OR users.user_id LIKE ?");
